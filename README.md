# 🍪 CookieSyncer – 通用 Cookie/TOKEN 同步助手

跨站点、跨设备的 Cookie/TOKEN 自动续期与同步助手，让登录状态永远在线。
...
- **🔧 域名管理**：支持手动添加域名配置，灵活控制同步范围
- **⏰ 定时任务**：基于 Chrome Alarms API，支持分钟级定时同步
- **🔄 多层续期策略**：
...
- **⚙️ 灵活配置**：支持自定义同步频率、续期策略、Cookie字段选择
- **🎨 美观界面**：现代化的弹窗和选项页面设计
...
#### 步骤4：验证安装成功
1. 扩展列表中出现"CookieSyncer – 通用 Cookie/TOKEN 同步助手"
2. 确保扩展已启用（右侧开关为蓝色）
...
4. **等待自动同步**：扩展会根据配置自动执行同步

#### 第三步：添加实际域名
1. **替换为实际域名**：如 `github.com`、`stackoverflow.com` 等
2. **调整配置**：根据网站特点调整同步频率和策略
3. **测试续期**：确认Cookie能够正确续期和同步
...
5. **安装完成**：
   - 扩展程序列表中会出现 "CookieSyncer – 通用 Cookie/TOKEN 同步助手"
   - 确保扩展程序已启用（右侧开关为蓝色）
...
- **启用/禁用**：控制是否参与自动同步
- **同步频率**：设置自动同步间隔（分钟，1-1440）
- **自动续期**：是否在同步时尝试续期 Cookie
- **续期策略**：选择续期失败时的处理策略
  - `fetch`：轻量级 HTTP 请求（推荐）
  - `tab`：后台打开页面
  - `script`：注入脚本交互
- **Cookie字段**：指定需要同步的Cookie字段名，支持多行配置
...
- **域名列表展示**：每个域名显示启用状态、Cookie数量、同步频率、上次同步状态
...
| **频率无法修改** | 1. 点击域名后的"编辑"按钮<br>2. 修改同步频率后点击"保存配置"<br>3. 等待几秒后刷新页面验证 |
| **扩展图标不显示** | 1. 确认扩展已正确加载<br>2. 检查是否启用了扩展<br>3. 刷新扩展管理页面 |
| **日志为空** | 1. 等待自动同步执行<br>2. 或手动点击"同步全部"按钮<br>3. 查看是否有错误日志 |
